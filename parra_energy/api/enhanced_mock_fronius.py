"""
Enhanced Mock Fronius API Client Module

This module provides a sophisticated mock client that simulates Fronius solar inverter data
using REAL weather data and historical patterns for optimal elderly user advice.

The enhanced mock generates realistic solar production patterns using:
- Real weather data from Open-Meteo API for Agramunt, Spain
- Historical consumption patterns from the database
- Actual solar production calculations based on weather conditions
- Elderly-specific usage patterns and recommendations

This ensures elderly users get the same quality advice whether using real or mock data.
"""
...
class EnhancedMockFroniusClient:
    def __init__(self, host: str = "mock", port: int = 80, db_path: str = "data/energy_data.db"):
        ...
        Args:
            ...
            db_path: Path to the database containing weather and historical data
        """
        self.host = host
        self.port = port
        self.db_path = db_path
        
        # System configuration (5kW solar system matching your setup)
        self.max_solar_power = 2200  # 2,2kW peak capacity - Real solar panels capacity
        
        # Manual solar percentage control for testing specific cases
        self._manual_solar_percentage = None  # None = automatic, 0-100 = manual override
        self._manual_consumption_watts = None  # None = automatic, number = manual override
        ...
        # Initialize weather service for real weather data
        self._init_weather_service()
        
        # Load historical patterns for elderly users
        self._load_historical_patterns()
        
        print("✅ Enhanced Mock using REAL weather data from Agramunt, Spain")
        
    def _init_weather_service(self):
        """Initialize connection to real weather service"""
        try:
            from parra_energy.weather.weather_service import WeatherService
            self.weather_service = WeatherService(self.db_path)
            self._has_weather_service = True
            print("🌤️  Connected to real weather service for Agramunt")
        except Exception as e:
            print(f"⚠️  Could not load weather service: {e}")
            print("🔄 Using fallback weather simulation")
            self.weather_service = None
            self._has_weather_service = False
    
    def _load_historical_patterns(self):
        """Load historical consumption patterns for elderly users"""
        self.elderly_patterns = {
            'morning_routine': {'time': (6, 10), 'factor': 1.8, 'activities': ['coffee', 'breakfast']},
            'midday_cooking': {'time': (12, 14), 'factor': 2.2, 'activities': ['lunch', 'cooking']},
            'evening_routine': {'time': (17, 21), 'factor': 2.5, 'activities': ['dinner', 'tv']},
            'night_rest': {'time': (21, 6), 'factor': 0.6, 'activities': ['sleep', 'minimal_usage']}
        }
        
        # Load actual historical data if available
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # Get average consumption patterns by hour from historical data
            cursor.execute('''
                SELECT 
